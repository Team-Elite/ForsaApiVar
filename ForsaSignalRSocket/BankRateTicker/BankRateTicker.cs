...
namespace ForsaSignalRSocket
{
    public class BankRateTicker
    {
        ...
        private readonly SqlTableDependency<BankRateInterestModel> _dependency;
        ...
        public BankRateTicker(Microsoft.AspNet.SignalR.Hubs.IHubConnectionContext<dynamic> clients)
        {
            ...
            var mapper = new ModelToTableMapper<BankRateInterestModel>().AddMapping(m => m.Id, "Id");
            _dependency = new SqlTableDependency<BankRateInterestModel>(connectionString: db.Database.Connection.ConnectionString, tableName:  "tblRateOfInterestOfBank", mapper: mapper);
        ...
        private void _dependency_OnChanged(object sender, RecordChangedEventArgs<BankRateInterestModel> e)
        {
            Clients.All.sendBankRate(e.Entity);
        }

        public  List<BankRateInterestModel> GetBankRate()
        {
            using (db = new forsawebEntities())
            {
                var Ratelist =  db.tblRateOfInterestOfBanks.ToList();

                return Ratelist.Select(f => new BankRateInterestModel()
                {
                    Id = f.Id,
