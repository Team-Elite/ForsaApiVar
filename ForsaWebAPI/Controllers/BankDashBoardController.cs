...
namespace ForsaWebAPI.Controllers
{
    ...
    public class BankDashBoardController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult GetAndUpdateResponseOfPendingRequestOnTheBasisOfRequestId(ApiRequestModel requestModel)
        {
            var data = new JwtTokenManager().DecodeToken(requestModel.Data);
            LenderSendRequestModel lenderRequestModel = JsonConvert.DeserializeObject<LenderSendRequestModel>(data);

            SqlParameter[] param = new SqlParameter[2];
            param[0] = new SqlParameter("@RequestId", lenderRequestModel.RequestId);
            param[1] = new SqlParameter("@UserTypeId", lenderRequestModel.UserTypeId);
            var dt = SqlHelper.ExecuteDataTable(HelperClass.ConnectionString, "USP_GetAndUpdateResponseOfPendingRequestOnTheBasisOfRequestId", System.Data.CommandType.StoredProcedure, param);
            if (dt == null || dt.Rows.Count == 0)
                return Json(new { IsSuccess = false });
            return Json(new { IsSuccess = true, data = new JwtTokenManager().GenerateToken(JsonConvert.SerializeObject(dt)) });
